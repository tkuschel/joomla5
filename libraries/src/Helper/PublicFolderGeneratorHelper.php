...
PHP;
    ...
    public function createPublicFolder(string $destinationPath): void
    {
        ...
        if ((!is_dir($fullDestinationPath) && !mkdir($fullDestinationPath, 0775, true))) {
            throw new \Exception('Unable to create the given folder, check the permissions.');
        }

        // Create the required folders
        if (
            !mkdir($fullDestinationPath . '/administrator/components/com_joomlaupdate', 0775, true)
            || !mkdir($fullDestinationPath . '/api', 0775, true)
        ) {
            throw new \Exception('Unable to create the given folder, check the permissions.');
        }
